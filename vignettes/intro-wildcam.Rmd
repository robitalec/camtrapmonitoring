...
land cover example data is an external TIF file that can be loaded with the
{terra} package. 

```{r}
# Load data
clearwater_lake_land_cover_path <- system.file('extdata', 'clearwater_lake_land_cover.tif', package = 'wildcam')
...
pts$lc_pt <- eval_pt(clearwater_lake_land_cover, pts)

# Evaluate distance to hydrology
pts$hydro_dist <- eval_dist(clearwater_lake_hydro, pts)

# Evaluate distance to wetland
pts$wetland_dist <- eval_dist(clearwater_lake_wetlands, pts)
...
- Maximum distance from road: 3000 m
...
Example data used in the {wildcam} package come from open data sources we 
